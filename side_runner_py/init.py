...
def initialize(driver_url):
    ...
    if Config.CHROMIUM_PATH is not None:
        chrome_options.binary_location = Config.CHROMIUM_PATH #"/tmp/bin/headless-chromium"
    if Config.CHROMEDRIVER_PATH is not None:
        driver = webdriver.Chrome(Config.CHROMEDRIVER_PATH, chrome_options=chrome_options)
    else:
